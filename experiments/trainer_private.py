...
class TrainerPrivate(object):
    ...
    def _local_update_noback(self, dataloader, num_labels, byz_mark, local_ep, lr, key, ind):
        ...
        for epoch in range(local_ep):
            ...
            for batch_idx, (x, y) in enumerate(train_ldr):
                ...
                if isinstance(key, dict):
                    ...
                    with torch.no_grad():
                        grad_need_cat=[param.grad.clone().cuda(self.device) for param in self.model.parameters()]
                        grad_cat_list=torch.cat([xx.reshape((-1, 1)).cuda(self.device) for xx in grad_need_cat], dim=0)
                    self.optimizer.step()
                    sign_loss_meter += sign_loss.item()    
        ...
        return self.model.state_dict(), grad_cat_list, np.mean(epoch_loss), sign_loss_meter, np.mean(local_acc)
